...
// extraSmall|small|medium|mediumLarge|large|extraLarge　という型ができる
...
//上記の型とstringの型を受け取れるようにしている。
//https://stackoverflow.com/questions/68807772/how-to-use-union-with-string-type-in-typescript
...
export function toPropValue<T>(
  ...
  //渡されたpropがResponsiveProp型である場合
  if (isResponsivePropType(prop)) {
    const result = [];
    //{base:,sm}という感じで値が入ってきて、responsiveKey=base というkeyが入ったりする
...
function toThemeValueIfNeeded<T>(propKey: string, value: T, theme?: AppTheme) {
  ...
  //themeにないものはそのままのstyleが変える。"8px"みたいな感じ
  return value;
}

//undefinedでなければpropsが返ってくる。なければfalse
...
function isSpaceThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
  //Object.keysをリストですると[0,1,2,3]とかの配列がわたる
